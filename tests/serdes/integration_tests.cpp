...
TEST(Integration, CoreRoundTripVector) {
    const std::vector<std::string> original = {"simple", "with,comma", "with\\backslash",
                                               "with\nnewline", "with\rcarriage"};

    const std::string serialized = serdes::serialize(original, ',', '\\');
    const auto round = serdes::deserialize(
        serialized.back() == '\n' ? serialized.substr(0, serialized.size() - 1) : serialized, ',',
        '\\');
...
TEST(Integration, CommandRoundTrip) {
    ...
    std::istringstream inStream("field1\n"
                                "fi;eld2\n"
                                "fi\\eld3\n");
